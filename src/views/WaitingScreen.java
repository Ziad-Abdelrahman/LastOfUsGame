...
public class WaitingScreen extends VBox implements Runnable{
	private Button back = new Button("Back");
	private FadeTransition fadeOut;
	public boolean joined;
	public Timeline timeline;
	public Client client;
	public SelectHeroMp select;
	public boolean running;
	public Thread run,listen;
	String name;
	public WaitingScreen(Client client,String name) {
		this.name=name;
		this.client=client;
...
@Override
	...
	public void listen() {
		listen = new Thread() {
			public void run() {
				while (running) {
					Object num = client.receive();
					
					System.out.println("WaitingSreen : "+num);
					if(Integer.parseInt((String) num)==2) {
						running=false;
						Platform.runLater(new Runnable() {
							...
							public void run() {
								...
								select = new SelectHeroMp(client,name);
