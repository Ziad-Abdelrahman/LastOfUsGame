...
public class SelectHero extends VBox{
	...
	public SelectHero() {
		...
		next.setOnMouseClicked(e->{
			...
			SelectHero.this.generateGame();
		});
		
		back.setOnMouseClicked(e->{
			Frame.backfx.seek(new Duration(0));
	    	Frame.backfx.play();
			SelectHero.this.back();
		});
	...
	public void selectRight() {
	    ...
	    SelectHero.this.heroselect.getChildren().remove(currentHero);
	    selectedindex++;
	    if (selectedindex > Game.availableHeroes.size() - 1)
	        selectedindex = 0;
	    nextHero = heroesPics.get(selectedindex);
	    SelectHero.this.heroselect.getChildren().add(1,nextHero);
	...
	public void generateGame() {
		c=true;
		Game.startGame(Game.availableHeroes.get(selectedindex));
		fadeOut.play();
